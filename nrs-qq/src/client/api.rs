    ...
    P: ClientProvider,
{
    ...
    pub async fn send_group_message(
        ...
        match P::TP::timeout(core::time::Duration::from_secs(5), rx).await {
            Ok(Ok(seq)) => {
    ...
    // /// 通过群号获取群
    pub async fn find_group(&self, code: i64, auto_reload: bool) -> Option<Arc<Group<P::RLP>>> {
    ...
    pub async fn reload_group(&self, group_code: i64) -> RQResult<()> {
        ...
        groups.insert(
            ...
            // }),
            Arc::new(Group::<P::RLP>::new(
                group_info,
                <P::RLP as RwLockProvider>::RwLock::new(members),
            )),
        );
        Ok(())
    }
    ...
    pub async fn reload_groups(self: &Arc<Self>) -> RQResult<()> {
        ...
        let group_list: Vec<(i64, Arc<Group<P::RLP>>)> = stream::iter(groups)
            .map(|g| async move {
                ...
                (
                    ...
                    Arc::new(Group::<P::RLP>::new(
                        g,
                        <P::RLP as RwLockProvider>::RwLock::new(mem_list),
                    )),
                )
            })
            .buffered(10)
